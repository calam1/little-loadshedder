...
impl LoadShedConf {
    fn new(ewma_param: f64, target: f64) -> Self {
        #[cfg(feature = "metrics")]
        {
            ...
            gauge!("loadshedder.capacity", "component" => "service").set(1.0);

            // gauge!("loadshedder.capacity", 1.0, "component" => "queue");
            gauge!("loadshedder.capacity", "component" => "queue").set(1.0);

            // gauge!("loadshedder.size", 0.0, "component" => "service");
            gauge!("loadshedder.size", "component" => "service").set(0.0);

            // gauge!("loadshedder.size", 0.0, "component" => "queue");
            gauge!("loadshedder.size", "component" => "queue").set(0.0);

            // gauge!("loadshedder.average_latency", target);
            gauge!("loadshedder.average_latency").set(target);
        }
        Self {
            target,
            ewma_param,
    ...
    async fn start(&self) -> Result<Permit, ()> {
        {
            ...
            gauge!("loadshedder.capacity", "component" => "queue")
                .set(desired_queue_capacity as f64);
    ...
    fn stop(&mut self, elapsed: Duration, concurrency_permit: Permit) {
        ...
        histogram!("loadshedder.latency").record(elapsed);
        ...
        gauge!("loadshedder.average_latency").set(stats.average_latency);
            ...
            && at_max_concurrency
        {
            ...
            if negative_gradient || (stats.average_latency > self.target) {
                ...
                if stats.concurrency > 1 {
                    ...
                    gauge!("loadshedder.capacity", "component" => "service")
                        .set(stats.concurrency as f64);
...
impl Permit {
    ...
    fn new(permit: OwnedSemaphorePermit, component: &'static str) -> Self {
        ...
        gauge!("loadshedder.size", "component" => component).increment(1.0);
        Self {
            permit: Some(permit),
            component,
        }
...
impl Drop for Permit {
    fn drop(&mut self) {
        ...
        gauge!("loadshedder.size", "component" => self.component).decrement(1.0);
    }
}
        ...
        Box::pin(async move {
            let permit = match conf.start().await {
                Ok(permit) => {
                    ...
                    counter!("loadshedder.request", "status" => "accepted").increment(1);
                    permit
                }
                Err(()) => {
                    #[cfg(feature = "metrics")]
                    // increment_counter!("loadshedder.request", "status" => "rejected");
                    counter!("loadshedder.request", "status" => "rejected").increment(1);
